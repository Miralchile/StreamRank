...
| 实验 | 注意力类型 | 多任务结构 | 回答的问题 |
|---|---|---|---|
| DeepFM式基线 | 无(隐式特征交叉) | Shared-bottom | 普通特征交叉能达到什么水平? |
| DIN | 目标注意力(序列) | Shared-bottom | 候选相关兴趣是否有效? |
| DIN+MMoE | 目标注意力(序列) | 4 Experts/Gates | 任务共享与分离是否继续提升? |
| SASRec式 | 因果自注意力(序列) | Shared-bottom | 自回归序列编码是否优于目标注意力? |
| BST式 | 序列自注意力(候选参与) | Shared-bottom | 候选进入注意力是否有增益? |
| AutoInt式 | 特征域自注意力(非序列) | Shared-bottom | 注意力用于特征交互而非序列是否有效? |

所有模型使用相同时间切分、词表、特征、任务权重和早停指标,参数量控制在
389k–427k同一量级。任何结构不会仅凭参数更多被宣布有效;必须同时报告参数量、
验证集选择结果和最终测试结果。
...
在同一协议下将对照扩展到六模型(`make rank-transformers`,3 seed × 6 模型全部
在同一环境重训;逐seed报告与汇总见
[`artifacts/transformer-comparison/comparison.json`](artifacts/transformer-comparison/comparison.json)):

| 模型 | 注意力类型 | 参数量 | 验证选型GAUC(3 seed 均值±σ) |
|---|---|---:|---:|
| DeepFM式 | 无(隐式交叉) | 389k | 0.6355 ± 0.0039 |
| DIN | 目标注意力(序列) | 398k | 0.6357 ± 0.0031 |
| DIN+MMoE | 目标注意力+专家 | 415k | 0.6326 ± 0.0007 |
| SASRec式 | 因果自注意力(序列) | 427k | 0.6298 ± 0.0020 |
| BST式 | 序列自注意力(含候选) | 427k | 0.6338 ± 0.0013 |
| AutoInt式 | 特征域自注意力(非序列) | 408k | **0.6431 ± 0.0031** |

两个结论。第一,序列自注意力延续并强化了前述负结果:SASRec式在验证集上显著差于
DeepFM(bootstrap Δ=+0.0090,95% CI [+0.0028, +0.0152]),BST式与DeepFM/DIN不可分;
"Pure的候选池内历史支撑不起序列建模"的判断由目标注意力扩展到自注意力家族。第二,
唯一表现出跨seed一致优势的是**非序列的特征域注意力(AutoInt式)**:3/3 seed胜出,
对DIN的选型GAUC差距-0.0112(CI [-0.0169, -0.0054]不含零),对DeepFM为-0.0059
(CI [-0.0124, +0.0004],96.6%重采样为正),测试集选型GAUC亦最高(0.6246,DeepFM
0.6206)。该优势体现在用户级GAUC上;全局Click AUC仍以DeepFM最高(0.7266 vs
...
- 在同协议、同参数量级下完成特征交叉/特征域注意力/目标注意力/序列自注意力的
  六模型谱系对照,全部结论附配对bootstrap置信区间与多seed稳定性;
- 发现序列建模(目标注意力与自注意力)在Pure截断历史上均无优势,而非序列的
