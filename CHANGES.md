...
# 2026-07-26 在线演示区数据增强(移除研究历程区块)

- 移除前端"研究历程"时间线区块(表述效果不佳;过程叙事由 docs/ 内的演示 deck 承担);
- 在线演示区四项增强,全部来自真实接口数据:每条结果的四目标概率迷你条、
  反馈后排名变化徽章(新进入/↑/↓/持平)、请求后自动刷新的引擎延迟指标条
  (/metrics 的 p50/p95/p99 与累计请求)、由部署权重动态生成的排序分公式行
  (/recommend 新增 score_policy 字段)。
